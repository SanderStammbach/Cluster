...
def solve_liouvillian_expm_exp(
    H: Qobj,
    rho0: Qobj,
    tlist: np.ndarray,
    c_ops: List[Qobj],
    ...
    # --- Erwartungswerte-Container EINMAL anlegen, konsistent und komplex ---
    expt: Optional[Dict[int, np.ndarray]] = None
    if e_ops:
        expt = {i: np.empty(len(tlist), dtype=np.complex128) for i in range(len(e_ops))}
    ...
    if is_uniform:
        ...
        for k, vk in enumerate(res):
            ...
            if expt is not None:
                for i, op in enumerate(e_ops):
                    # KEIN float(...): komplex zulassen
                    expt[i][k] = expect(op, rho)
            if k % Auslassen == 0:
                states.append(rho)
    else:
        t_prev = tlist[0]
