...
def TCLsolve_TCL2(
    ...
    L_of_t = build_L_TCL2_time_dependent(
        ...
        G=G ,Gc=Gc, use_secular=use_secular, dw_min=dw_min
    )
...
def make_G_from_Gamma(Gamma, W, t_round=8, cache_size=50_000):
    ...
    def G(r, i, j, t):
        t_key = round(float(t), t_round) # so hats funktioniert. jtz nichmm ich mal testweise ungerundet
        t_key = float(t)
        return Gamma_cached(t_key, float(W[i, j]))

    def Gc(r, i, j, t):
        # exakt das, was in der Formel steht: komplex konjugiert
        t_key = round(float(t), t_round)# gleiche hier
        t_key = float(t)
