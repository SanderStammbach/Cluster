...
N=46 # Größe Hilbertraum von vibronic system
...
#ax.text(-0.090, 0.80, r"$\frac{\mathrm{Tr}[\mathcal{H}_{B free} \hat{\rho}(t)]}{ n \omega_B }$", transform=ax.transAxes,rotation=0,fontsize=23, va='bottom', ha='center')
ax.set_ylabel(r"$\frac{Energy} {  n \omega_B}$",fontsize=23,rotation=0,labelpad=35  )
...
for i in range(8):
    ...
    ax.text(
        n[i], 
        filled_ergotropy[i] + total_energy[i] * 0.02,  # leichter Abstand nach oben
        f'{percent:.1f}%', 
...
for i in range(8):
    ...
    ax.text(
        n[i], 
        filled_energy[i] + total_energy[i] * 0.02,  # leichter Abstand nach oben
        f'{percent:.1f}%', 
        ha='center', va='bottom', fontsize=16, fontweight='bold'
    )
...
plt.savefig("Battery.png", dpi=300, bbox_inches="tight")
plt.show()
