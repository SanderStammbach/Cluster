...
def grid_maker():
    vec1 = np.linspace(0, 7, 30,dtype=np.float64)                     # y-Achse (Δ)
    vec2 = np.linspace(-8, 8 , 30,dtype=np.float64)  # x-Achse (λj)
...
# Titel
ax.set_title(r"$Var(b)$", fontsize=18)
...
pcm = ax.pcolormesh(X, Y, Z, shading='auto', cmap='PRGn' )
...
plt.savefig("OccupationProb_variance.png",dpi=300, bbox_inches="tight", pad_inches=0.2)
plt.show()
